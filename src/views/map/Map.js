...
class Map extends Component {
  ...
  initPanel() {
    ...
    ReactDOM.render(
      <PhotoUpload okCallback={this.uploadCallback.bind(this)} />,
  ...
  uploadCallback(list) {
    let markers = list.filter(item => !!item.exifData).map(item => {
      let marker = new maptalks.Marker(item.exifData.coord).addTo(
        this.map.getLayer('v')
      )
      marker.setInfoWindow({
        content: `<div class="photo-box"><img src="${item.base64}" /></div>`,
        minWidth: 120
        // 'autoPan': true,
        // 'width': 300,
        // 'minHeight': 120,
